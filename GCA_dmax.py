...
class CS12s:
    ...
    def parse(self, n, al, p=0.5, _p=0.5, pt=None, _pt=None, pc=None, _pc=None, p4=None, _p4=None, update=False):
        ...
        prev_list = set()
        if update:
            degree_sequence = sorted([d for n, d in al.degree()], reverse=True)
            #d_max = max(degree_sequence) #maximum degree for common neighbors
            d_max = 10
            """class-1 statistics"""
            ab = arr([0,0]) # keep track of KTe for p
            abt = arr([0,0]) # keep track of KTe for pt
            abc = np.zeros([d_max+1,2]) # keep track of KTe for common neighbors coder
            ab4 = np.zeros([4,2]) # keep track of KTe for 4-node motifs
            p = 0.5
            pt = 0.5
            pc = 0.5*np.ones(d_max+1)
            p4 = 0.5*np.ones(4)
            """"class-2 statistics"""
            _ab = arr([0,0]) # keep track of KTe for p
            _abt = arr([0,0]) # keep track of KTe for pt
            _abc = np.zeros([d_max+1,2]) # keep track of KTe for common neighbors coder
            _ab4 = np.zeros([4,2]) # keep track of KTe for 4-node motifs
            _p = 0.5
            _pt = 0.5
            _pc = 0.5*np.ones(d_max+1)
            _p4 = 0.5*np.ones(4)
            
        for k in range(0,n-1):
            #if not k%500: print('node:{0}, time:{1}'.format(k, timer()))
            #print()
